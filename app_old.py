...
def society_send():
    ...
    subject = f"【千艺界】社招版面试问卷 - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}"
...
def fresh_send():
    ...
    subject = f"【千艺界】应届生版面试问卷 - {data.get('applicant_name','未署名')} - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}"
    ok, err = send_summary_email(subject, summary)
    if ok:
        return {"status": "sent"}
    else:
